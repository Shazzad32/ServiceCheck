...
export const PUT = async (req) => {
  try {
    ...
    // Update the document using the entire item
    const updatedServiceCheck = await ServiceCheck.findByIdAndUpdate(
      item._id, // Use item._id to find the document
      item, // Pass the entire item object for updating
      { new: true }
    );
    ...
    console.log("Error in PUT request:", error); // Log detailed error
    return new Response(error.message, { status: 500 });
  }
};
